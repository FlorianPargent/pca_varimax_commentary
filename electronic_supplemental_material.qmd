...
phonedata_sensing = as.data.table(phonedata_sensing)

na_cnames = colnames(phonedata_sensing)[
  phonedata_sensing[, sapply(.SD, anyNA)]]
notna_cnames = setdiff(colnames(phonedata_sensing), na_cnames)

for (i in na_cnames) {
  phonedata_sensing[, c(i) := fill_NA_N(x = .SD,
  model = "pmm",
  posit_y = i,
  posit_x = sample(notna_cnames, size = 10),
  k = 10)]
}
```
...
Combining the top `r n` items loading on all components results in only `r length(red_vars)` unique variables. 

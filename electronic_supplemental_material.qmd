...
top_vars_z = data.frame(
  ...
  Music_Frequency = names(sort(abs(pca_sensing_imp_z$Y[, 3]), decreasing = TRUE)[1:n]),
  Music_Energy = names(sort(abs(pca_sensing_imp_z$Y[, 4]), decreasing = TRUE)[1:n]),
...
Interestingly, R&Z show as a side result that in their semi-parametric factor model, their version of PCA+VM can be used to also estimate correlated factors und situations relevant for psychological measurement models.
...
Correlated factor scores are simulated by multiplying the uncorrelated scores with the Cholesky factor of a correlation matrix with the desired correlation.
...
```
When plotting the simulated component scores in the $Z$ matrix, we see that the two dimensions are now correlated (i.e. oblique components).

2. Simulate item response data.

We create a loading matrix $Y$ with simple structure and simulate the data matrix $A$ by $ZY^T$ plus some noise.
...
6. Estimate factor correlation ($\rho = `r rho`$)

...
To summarize, with careful preprocessing and appropriate data standardization, PCA+VM seems to meaningfully cluster the smartphone sensing variables, as indicated by radial streaks in the estimated loading matrix $\hat{Y}$.
However, we did not find streaks in the estimated component matrix $\hat{Z}$ so we cannot have confidence that PCA+VM was able to estimate well identified person scores on these components.
...
Our suspicion is that the radial streaks in the estimated component matrix $\hat{Z}$ might be a result of these GPS variables, which dominate the solution from PCA+VM without standardization but are hidden behind the remaining variables when standardization is used.

To confirm some of these suspicions, we repeat PCA+VM on only these `r length(red_vars)` variables, but now use standardization again.
...
Another important aspect of psychological applications of PCA+VM are correlated factors (e.g. the personality dimensions conscientiousness and neuroticism are thought to correlate around $-0.4$; @vanderlinden2010general), thus oblique rotations are often argued for.
Interestingly, R&Z show as a side result that the results of PCA+VM can be used to also estimate correlated factors in situations which are relevant for psychological measurement models.
...
When plotting the $\hat{Z}$ matrix returned from PCA+VM, we see orthogonal streaks similar to $Z^*$.
...
Our simulation demonstrates the results from sections **7.1** and **7.2** in R&Z, which state that if scores in $Z$ are correlated; scores in $Y$ are centered, independent, and leptokurtic; and the true $B$ is proportional to the identity matrix, then the $\hat{B}$ matrix returned from PCA+VM estimates the Cholesky factor of the covariance matrix $Z^TZ$.

...
Finally, we demonstrate R&Z's side result that the matrix $\hat{Z}\hat{B}$ from PCA+VR can correctly estimate person scores simulated from oblique leptokurtic components.
<!-- Finally, we demonstrate R&Z's surprising side result that PCA+VR can sometimes correctly estimate correlated person scores. -->

<!-- Although R&Z proof that latent variable models sometimes could be estimated by PCA+VR, the method not necessarily has its main use in this way. -->
...
Components are most useful when they predict other meaningful quantities, which does not require latent variables in any strict sense.

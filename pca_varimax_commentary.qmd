...
In our opinion, abandoning the simpler PCA(+VR) is a mistake and R&Z refute a common argument by proving that PCA+VM *can* perform statistical inference in latent variable models:
...
Finally, we demonstrate R&Z's surprising side result that the matrix $\hat{Z}\hat{B}$ from PCA+VR can correctly estimate person scores simulated from oblique leptokurtic components.
